...
namespace ClientAutoritative
{
    ...
    public class ShipController : PlayerController
    {
        ...
        void LateUpdate()
        {
            ...
            if (IsOwner)
            {
                lookAt.position = transform.position + body.velocity.normalized;
        ...
        protected Vector3 ApplyMotorAcceleration(float torqueApply, Vector3 localVelocity, Vector3[] localMatrix)
        {
            ...
            if (Mathf.Abs(torqueApply) > 0.1f)
            {
                ...
                var healthMod = ((1f - minHealthImpactOnSpeed) / player.MaxHealth) * player.Health.Value + minHealthImpactOnSpeed;
