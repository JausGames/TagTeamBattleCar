...
public class ShipController : NetworkBehaviour
{
    ...
    void Update()
    {
        ...
        if (IsServer) UpdateServer(localVelocity, torqueApply.Value > 0);
    ...
    private void UpdateClient(Vector3[] localMatrix, Vector3 localVelocity, Vector3 localAngularVelocity)
    {
        ...
        if (onFloor)
        {
            ...
            torque += ApplySteeringRotation(rotation.Value, localAngularVelocity);
        ...
        if (oldForce != force || oldTorque != torque) RequestUpdateServerRpc(force, torque);
    }
    [ServerRpc]
    void RequestUpdateServerRpc(Vector3 force, Vector3 torque)
    {
        this.force.Value = force;
        this.torque.Value = torque;
    }

    private void UpdateServer(Vector3 localVelocity, bool engineOn)
    {
    ...
    protected Vector3 ApplySteeringRotation(Vector3 rotation, Vector3 localAngularVelocity)
    {
        //Rotate the ship
        ...
        {
            if (Mathf.Sign(previousRotation.x) != Mathf.Sign(rotation.x)) body.angularVelocity = localAngularVelocity.x * Vector3.right + localAngularVelocity.z * Vector3.up;
            torque = rotation.x * carSettings.rotationSpeed * steerRatio * transform.up;
        }
        else body.angularVelocity = localAngularVelocity.x * Vector3.right + localAngularVelocity.z * Vector3.up;
