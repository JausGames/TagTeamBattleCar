...
public class LobbyHelloWorld : MonoBehaviour
{
    ...
    async Task ExecuteLobbyDemoAsync()
    {
        ...
        // Log in a Unity.Services.Lobbies.Models.Player for this game client
        Unity.Services.Lobbies.Models.Player loggedInPlayer = await GetPlayerFromAnonymousLoginAsync();

        // Add some data to our Unity.Services.Lobbies.Models.Player
        // This data will be included in a lobby under players -> Unity.Services.Lobbies.Models.Player.data
        ...
        // Order results by available Unity.Services.Lobbies.Models.Player slots (least first), then by lobby age, then by lobby name
        ...
        if (foundLobbies.Any()) // Try to join a random lobby if one exists
        {
            ...
            // Unity.Services.Lobbies.Models.Player is optional because the service can pull the Unity.Services.Lobbies.Models.Player data from the auth token
            // However, if your Unity.Services.Lobbies.Models.Player has custom data, you will want to pass the Unity.Services.Lobbies.Models.Player object into this call
        ...
        // Let's add some new data for our Unity.Services.Lobbies.Models.Player and update the lobby state
        ...
        // Now, let's try the QuickJoin API, which just puts our Unity.Services.Lobbies.Models.Player in a matching lobby automatically
        ...
        try
        {
            ...
            {
                Player = loggedInPlayer, // Including the Unity.Services.Lobbies.Models.Player here lets us join with data pre-populated
    ...
    // Log in a Unity.Services.Lobbies.Models.Player using Unity's "Anonymous Login" API and construct a Unity.Services.Lobbies.Models.Player object for use with the Lobbies APIs
    static async Task<Unity.Services.Lobbies.Models.Player> GetPlayerFromAnonymousLoginAsync()
    {
        if (!AuthenticationService.Instance.IsSignedIn)
        {
            ...
            if (!AuthenticationService.Instance.IsSignedIn)
            {
                throw new InvalidOperationException("Unity.Services.Lobbies.Models.Player was not signed in successfully; unable to continue without a logged in Unity.Services.Lobbies.Models.Player");
            }
        }

        Debug.Log("Unity.Services.Lobbies.Models.Player signed in as " + AuthenticationService.Instance.PlayerId);

        // Unity.Services.Lobbies.Models.Player objects have Get-only properties, so you need to initialize the data bag here if you want to use it
